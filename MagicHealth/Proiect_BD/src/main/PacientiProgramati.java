...
public class PacientiProgramati extends JFrame implements ActionListener {
    private JList listPacienti;
    ...
    public PacientiProgramati(String username, String role, Connection connection) {
        this.uid = username;
        this.role = role;
        this.con = connection;
        ...
        labelMessage.setText("Pacienti programati pentru ziua de azi, " + now().toString() + ":");
        if (role.equals("medic")) {
            try {
                String statement = "CALL vizualizare_pacienti_programati(?, ?);";
                String statement2 = "SELECT * FROM medic, users WHERE medic.cnp = users.cnp_user AND users.username = '" + uid + "';";

                Statement find = con.createStatement();
                ResultSet found = find.executeQuery(statement2);
                found.next();

                CallableStatement stmt = con.prepareCall(statement);
                stmt.setString(1, now().toString());
                stmt.setInt(2, found.getInt("cnp"));
                ResultSet rst = stmt.executeQuery();
                List<String> list = new ArrayList<String>();

                while(rst.next()) {
                    list.add(rst.getString("nume") + " " + rst.getString("prenume") + " "  + rst.getString("data_ora") +
                          " " + rst.getString("id_policlinica") + " " + rst.getString("nr_cabinet") + " " +
                            rst.getString("durata"));
                }

                listPacienti.setListData(list.toArray());
            ...
            try {
            String statement = "SELECT * FROM programare WHERE data_ora LIKE '" + now() + "%';";
            Statement stmt = con.createStatement();
            ResultSet rst = stmt.executeQuery(statement);
            List<String> list = new ArrayList<String>();

            while(rst.next()) {
                list.add(rst.getString("id") + " data: " + rst.getString("data_ora") + " medic: " + rst.getString("id_medic") + " pacient: " +
                        rst.getString("id_pacient") + " policlinica: " + rst.getString("id_policlinica") +
                        " cabinet: " + rst.getString("nr_cabinet") + " serviciul: " +
                        rst.getString("id_serviciu"));
            }
