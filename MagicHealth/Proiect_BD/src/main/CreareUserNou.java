...
public class CreareUserNou extends JFrame implements ActionListener {
    ...
    public CreareUserNou(String user, String role, Connection connection) {
        this.uid = user;
        this.role = role;
        this.con = connection;
        ...
        salvareButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                //se preiau datele care urmeaza a fi introduse in baza de date
                String cnp = textFieldCNP.getText();
                String newUser = textFieldUsername.getText();
                String newPass = textFieldParola.getText();
                String newFunction = textFieldFunctie.getText();

                if(cnp.isEmpty() || newUser.isEmpty() || newPass.isEmpty() || newFunction.isEmpty()) {
                    JOptionPane.showMessageDialog(mainPanel, "Completati toate campurile");
                }
                else {
                    try {
                        String statement = "SELECT * FROM angajat WHERE cnp = " + textFieldCNP.getText() + ";";
                        Statement stmt = con.createStatement();
                        ResultSet rst = stmt.executeQuery(statement);
                        if (!rst.next()) {
                            JOptionPane.showMessageDialog(mainPanel, "Nu exista angajat cu cnp-ul specificat");
                        } else {
                            if (role.equals("admin") && textFieldFunctie.getText().equals("admin")
                                    || textFieldFunctie.getText().equals("super-admin")) {

                                JOptionPane.showMessageDialog(mainPanel, "Insuficiente privilegii pentru a crea un administrator");
                            } else {
                                //daca nu se depisteaza erori, are loc apelul de procedura
                                String statement2 = "CALL user_nou(?, ?, ?, ?);";
                                CallableStatement stmt2 = con.prepareCall(statement2);
                                stmt2.setInt(1, rst.getInt("cnp"));
                                stmt2.setString(2, textFieldFunctie.getText());
                                stmt2.setString(3, textFieldUsername.getText());
                                stmt2.setString(4, textFieldParola.getText());
